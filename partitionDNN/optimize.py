...
import time
from itertools import product   # product：组合迭代器，product(A, B)理解为迭代其笛卡尔积
...
#################################################################下面这个循环存疑############################################################
# 枚举所有可能的情况，即各个时刻同一边缘设备开关与否的组合
for item in product(*keys):
    locals.append([int(v) for i, v in enumerate(item)])     # enumerate：返回参数的（index， value）
