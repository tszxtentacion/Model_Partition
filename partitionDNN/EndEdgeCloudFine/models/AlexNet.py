...
def add_watermark(watermark):
    ...
    state_dict = torch.hub.load_state_dict_from_url(file, progress=True)
    start_t = time.time()
    for i, name in enumerate(state_dict):
        print(state_dict[name].shape)
        if i==14:
            print(state_dict[name])
        if len(state_dict[name].shape) == 4:
                ...
                for j in i:
                    for x in j:
                        for k in x:
                            hash(k)
                            # print(hash(k))
        elif len(state_dict[name].shape) == 1:
            for i in state_dict[name]:
                hash(i)
                # print(hash(i))
        elif len(state_dict[name].shape) == 2:
            for i in state_dict[name]:
                for j in i:
                    hash(j)
                    # print(hash(j))
    ...
    state_dict["watermark"] = watermark
    torch.save(state_dict, "./water_model.pth")



def alexnet(num_classes=1000, pretrained=False, progress=True):
    file = 'https://download.pytorch.org/models/alexnet-owt-4df8aa71.pth'
...
if __name__ == '__main__':
    ...
    for img_size in imgSize:
        ...
        input_shapes = []
        output_shapes = []
        summaries = summary(models.alexnet(False), input_size=(3, img_size, img_size), batch_size=-1,device='cpu')
        ...
        # 执行features
        intermediate = execute_layers(intermediate, "features", 13, 0, img_size)
        ...
        # print("============")
        times.append(t)
        layers.append(layer + "%s" % 13)
        output_data_sizes.append(output_data_size)
        input_data_sizes.append(input_data_size)

        # classifier
        intermediate = execute_layers(intermediate, "classifier", 7, 1, img_size)
